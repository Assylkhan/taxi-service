...
public class H2ClientDao implements ClientDao {
    ...
    private static final String SELECT_BY_COMMENT_ID = "SELECT * FROM COMMENT" + COMMENT_JOIN +
            "WHERE COMMENT.ID=?";
    private static final String UPDATE_BY_ID = "UPDATE CLIENT SET FIRST_NAME=COALESCE(?, FIRST_NAME), " +
            "LAST_NAME=COALESCE(?, LAST_NAME), LOGIN=COALESCE(?, LOGIN), " +
            "PASSWORD=COALESCE(?, PASSWORD), BONUS=COALESCE(?, BONUS) WHERE ID=?";
    ...
    public void update(Client client) throws DaoException {
        try(PreparedStatement statement = connection.prepareStatement(UPDATE_BY_ID)){
            ...
            statement.setBigDecimal(5, client.getBonus());
    ...
    public Client getClientBean(ResultSet resultSet) throws SQLException {
        ...
        client.setBonus(resultSet.getBigDecimal("BONUS"));
