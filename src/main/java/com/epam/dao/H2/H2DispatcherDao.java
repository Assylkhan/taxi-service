...
public class H2DispatcherDao implements DispatcherDao {
    ...
    private static final String SELECT_BY_CREDENTIALS = "SELECT * FROM DISPATCHER " +
            "WHERE DISPATCHER.LOGIN = ? AND DISPATCHER.PASSWORD = ?";
    private static final String INSERT = "INSERT INTO DISPATCHER " +
            "(LOGIN, PASSWORD, FIRST_NAME, LAST_NAME, NATIONAL_ID, PHONE) VALUES (?, ?, ?, ?, ?, ?)";
    private static final String SELECT_ALL = "SELECT * FROM DISPATCHER";
    private static final String COUNT = "SELECT COUNT(*) FROM DISPATCHER";
    private static final String SELECT_BY_NATIONAL_ID = "SELECT * FROM DISPATCHER WHERE NATIONAL_ID=?";
    ...
    public Dispatcher findByCredentials(String login, String password) throws DaoException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(SELECT_BY_CREDENTIALS)) {
    ...
    public List<Dispatcher> findAll() throws DaoException {
        try (Statement statement = connection.createStatement();
            ...
            while (resultSet.next()) {
                dispatchers.add(getDispatcherBean(resultSet));
            }
            return dispatchers;
        } catch (SQLException e) {
            throw new DaoException(e);
    ...
    @Override
    public void isExistsNationalId(String nationalId) throws DaoException {
        try (PreparedStatement statement = connection.prepareStatement(SELECT_BY_NATIONAL_ID)) {
            statement.setString(1, nationalId);
            if (statement.execute() == false) throw new SQLException("such national id already exists");
        } catch (SQLException e) {
            throw new DaoException(e);
        }
    }
